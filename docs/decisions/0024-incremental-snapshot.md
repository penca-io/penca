...
**Amended by CHA-531 — the gate is catalog-wide, and the grace clock is a cross-branch max.** Carry-forward crosses fork edges too: a fork's first snapshot carries the parent's untouched partitions, so a live reference to an `object_uri` can sit in *any* branch's partition, not only the sweeping branch's. Both refcount probes therefore drop their `branch_uuid` filter. A third arm joins `segment_delete_set` to itself on `object_uri`, because `naming::segment_delete_uuid` is branch-keyed — the retirement that drops the last reference refreshes only the retiring branch's row, so without it the parent's already-expired row would go eligible the instant a child dropped that reference. Branch teardown is excluded: `drop_branch_partitions` CASCADEs the child's segment and delete-set partitions together, and readers of a dropped branch are fenced by DeleteBranch rather than by the grace window. See `eligible_segment_delete_set_rows` for the cost profile the widening implies.
