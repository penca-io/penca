...
**Amended by CHA-531 — the gate and the queue are both catalog-wide.** Carry-forward crosses fork edges too: a fork's first snapshot carries the parent's untouched partitions, so a live reference to an `object_uri` can sit in *any* branch's partition, not only the sweeping branch's. Both refcount probes therefore drop their `branch_uuid` filter, and `segment_delete_set` is re-keyed on `object_uri` alone — one row per file for the whole catalog, unpartitioned. The single row is what keeps the grace clock honest: the retirement that drops the last reference refreshes that one row's `written_at_micros`, where branch-keyed rows would have left the parent's already-expired row eligible the instant a child dropped its reference. Branch teardown needs no delete-set partition to drop — `drop_branch_partitions` removes the branch's segment metadata, which drops its references, and the refcount gate then lets the sweep collect the file on its own. See `eligible_segment_delete_set_rows` for the cost profile the widening implies.

**Amended by CHA-539 — the persist tier joins the refcount domain.** A fork materializes its inherited cold references as `table_persist_segment_metadata` rows naming the parent's files, and branch teardown became enqueue-only, so it queues those same URIs. A third `NOT EXISTS` arm over the persist segment table is therefore load-bearing: without it the sweep collects a file a sibling branch still reads. This is the tier's *first* cross-branch protection, not a second — the [CHA-72](https://linear.app/chapala/issue/CHA-72) `earliest_fork_point_into` retention clamp it was previously attributed to is unimplemented. Both tiers are now refcounted and the reference domain is uniform.
