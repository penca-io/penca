...
/// dispatch) talks exclusively to the microservices over gRPC.
...
/// concurrent strict-INSERTs across SQL clients (ADR 0006).
pub struct ServerConfig {
    ...
    /// `x-penca-catalog` header is supplied**. The header
    ...
    /// read from `SessionConfig.options.catalog.default_schema`. Every
    /// catalog auto-creates a `public` schema at
    ...
    /// header is supplied**. The header wins when present;
    ...
    /// `main` branch, so out-of-the-box writes work without
    /// operator setup.
    pub default_branch: String,
    /// Per-connection logical-plan cache capacity. `GetFlightInfo`
