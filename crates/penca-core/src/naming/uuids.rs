...
// -- Persist + snapshot UUID chain (CHA-203) -------------------------------
//
...
/// Deterministic table_purge UUID for one `(branch, table, purged_at)`.
///
/// Rooted on a catalog-scoped `"purge"` anchor rather than on
/// `catalog_uuid` directly — same arity-3 PK tuple as
/// [`table_snapshot_uuid`] would otherwise collide. The
/// `deterministic_uuid_from(&[catalog_str, "purge"])` anchor keeps
/// the hash-input space disjoint without needing a discriminator tag
/// in the outer `row_uuid_for_pk` call.
...
mod tests {
    ...
    fn test_genesis_tx_uuid() {
        ...
        // Genesis tx UUID is derived from catalog UUID, should be deterministic
        assert_eq!(genesis, genesis_tx_uuid(&CAT));
        // But different from the catalog UUID itself
    ...
    //
    // CHA-236: namespace UUID hash helpers were deleted (catalog/schema/
    // branch/table UUIDs are now server-minted random). The literal
    // `CAT` / `BR` / `TBL` consts above reproduce the pre-CHA-236
    // outputs so chain-helper goldens stay comparable; the partition
    // helpers and `table_purge_uuid` have new goldens reflecting
    // the re-rooted hash inputs.
