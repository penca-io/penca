...
description: kata create/ready --json expose .issue.short_id (no qualified_id); build refs as penca#<short_id>; --blocked-by rejects bare numeric ids
type: reference
---
...
- `kata list --json` → `.issues[].qualified_id` **is** present (e.g. `penca#mtfc`), alongside `.labels` and `.blocked_by` (the latter an array of `{uid, short_id}` objects).

To capture a freshly-created task's ref, read `.issue.short_id` and build `penca#<short_id>` — reading `.qualified_id` off `create`/`ready` output yields empty and silently breaks any downstream `--blocked-by`/`--related` wiring.

Refs passed to `--blocked-by` (and friends) must be the qualified `penca#<short_id>` form (or the ULID). A bare numeric id is rejected: `"…looks like a legacy issue number; use a short_id (e.g. abc4) or kata#abc4"`.
