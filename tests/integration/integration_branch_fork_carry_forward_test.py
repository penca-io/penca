...
def test_fork_first_snapshot_watermark_stops_at_a_stranded_parent_commit():
    """The other side of the same claim: a parent commit BELOW the fork
    edge but ABOVE the snapshot's ``as_of`` pin is not materialized, so
    ``W_snap`` must stop short of the fork edge rather than claim it.

    Both plan paths clamp the parent on the micros axis as well as the seq
    one — ``assemble_fork_tail_base_cold``'s ``max_micros``,
    ``enumerate_base_cold_source``'s — so an ``as_of`` pin below such a
    commit strands it: it is filtered out of what the snapshot writes.
    Stamping the fork edge anyway would close ``meta_plan``'s base-cold
    gate over it and the row would vanish from the child for the life of
    the branch, since no later snapshot re-folds the parent (``fork_edge``
    is gated on the child having no prior snapshot).

    So this is the paired constraint to
    ``test_fork_first_snapshot_watermark_covers_the_fork_point``: that one
    forbids understating, this one forbids overstating. Only both together
    pin the watermark to what the snapshot actually holds.
    """
    client, catalog_uuid, schema_uuid, table_uuid, main_branch = (
        setup_partitioned_table("fcf")
    )
    ...
    # The pin lands here — after `dave`, before `eve`.
    as_of = _persist_watermark(catalog_uuid, main_branch, table_uuid)

    # `eve` is the stranded commit: under the fork edge on the seq axis,
    # over the pin on the micros axis.
    write_and_persist(
        client,
        catalog_uuid=catalog_uuid,
        schema_uuid=schema_uuid,
        table_uuid=table_uuid,
        branch_uuid=main_branch,
        upserts=pa.table({"name": ["eve"], "value": [5]}, schema=USER_SCHEMA),
    )
    assert _persist_watermark(catalog_uuid, main_branch, table_uuid) > as_of, (
        "`eve` must persist strictly after the pin or nothing is stranded"
    )

    branch = client.create_branch(
        f"fcf_str_{uuid4().hex[:6]}",
        catalog_uuid=catalog_uuid,
        author="test",
        comment="cha-531",
    )
    ...
    assert (
        ...
        "the snapshot does not hold `eve` — the pin filtered it out — so its"
        f" W_snap must stay under fork_commit_seq_num={branch.fork_commit_seq_num}."
        " Claiming the fork edge closes the base-cold gate over a parent commit"
        " the snapshot never materialized."
    )

    # The consequence the watermark exists to protect: `eve` is still
    # reachable, because the gate stayed open and the read folds the
    # parent's cold tier.
    assert _read_pairs(
        client,
        catalog_uuid=catalog_uuid,
        ...
        table_uuid=table_uuid,
        branch_uuid=child_branch,
    ) == [("alice", 99), ("bob", 2), ("carol", 3), ("dave", 4), ("eve", 5)], (
        "a parent commit stranded between the snapshot's pin and the fork edge"
        " was lost on the child"
    )
